...
func TestMainSendKeysUsageIncludesWaitReadyFlags(t *testing.T) {
	...
	if !strings.Contains(out, "usage: amux send-keys (<pane>|--window <index|name>) [--via pty|client] [--client <id>] [--wait ready|ui=input-idle] [--timeout <duration>] [--delay-final <duration>] [--submit] [--hex] <keys>...") {
...
func TestMainKeyCommandsHelpFlagsPrintUsage(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			want: "usage: amux send-keys (<pane>|--window <index|name>) [--via pty|client] [--client <id>] [--wait ready|ui=input-idle] [--timeout <duration>] [--delay-final <duration>] [--submit] [--hex] <keys>...",
		},
		{
