...
func TestParseDebugCommand(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:         "parses raw heap profile",
			loadConfig:   func() (*config.Config, error) { return &config.Config{Debug: config.DebugConfig{Pprof: true}}, nil },
			args:         []string{"heap", "--raw"},
		...
		{
			name:         "parses bare integer client profile duration as seconds",
			loadConfig:   func() (*config.Config, error) { return &config.Config{Debug: config.DebugConfig{Pprof: true}}, nil },
			args:         []string{"client-profile", "--duration", "20"},
			wantPath:     "/debug/pprof/profile?seconds=20",
		...
		{
			name:         "parses bare integer profile duration as seconds",
			loadConfig:   func() (*config.Config, error) { return &config.Config{Debug: config.DebugConfig{Pprof: true}}, nil },
			args:         []string{"profile", "--duration", "20"},
			wantPath:     "/debug/pprof/profile?seconds=20",
			wantTimeout:  25 * time.Second,
			wantSockPath: wantSocket,
			wantEnabled:  true,
