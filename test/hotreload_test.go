...
func newPersistentReloadHarness(tb testing.TB, binPath string) *AmuxHarness {
	tb.Helper()
	return newAmuxHarnessWithBin(tb, binPath, "AMUX_EXIT_UNATTACHED=0", "AMUX_NO_WATCH=0")
}

func newPersistentReloadHarnessInDir(tb testing.TB, binPath, launchDir string) *AmuxHarness {
	tb.Helper()
	return newAmuxHarnessWithBinInDir(tb, binPath, launchDir, "AMUX_EXIT_UNATTACHED=0", "AMUX_NO_WATCH=0")
}
