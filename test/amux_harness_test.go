...
func newAmuxHarness(t *testing.T) *AmuxHarness {
	t.Helper()
...
func (h *AmuxHarness) generation() uint64 {
	...
	if err != nil {
		h.t.Fatalf("parsing inner generation: %v (output: %q)", err, out)
	}
	return n
}
...
func (h *AmuxHarness) waitLayout(afterGen uint64) {
	...
	if strings.Contains(out, "timeout") {
		h.t.Fatalf("inner wait-layout timed out after generation %d\ncapture:\n%s", afterGen, h.capture())
	}
}
...
func (h *AmuxHarness) splitV()     { h.t.Helper(); h.doSplit("\\") }
func (h *AmuxHarness) splitH()     { h.t.Helper(); h.doSplit("-") }
func (h *AmuxHarness) splitRootV() { h.t.Helper(); h.doSplit("|") }
func (h *AmuxHarness) splitRootH() { h.t.Helper(); h.doSplit("_") }
